...
public class InsidePaymentServiceImpl implements InsidePaymentService {
    ...
    public Response pay(PaymentInfo info, HttpHeaders headers) {
        ...
        if (info.getTripId().startsWith("G") || info.getTripId().startsWith("D")) {
            requestOrderURL =  order_service_url + "/api/v1/orderservice/order/{orderId}";
        } else {
            requestOrderURL = order_other_service_url + "/api/v1/orderOtherService/orderOther/{orderId}";
        }
    ...
    public Response queryAddMoney(HttpHeaders headers) {
        ...
        } else {LOGGER.error("[queryAddMoney][Query money failed][addMoneyRepository.findAll null]");
